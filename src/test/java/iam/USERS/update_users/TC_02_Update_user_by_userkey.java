...
public class TC_02_Update_user_by_userkey extends base {
	
	String userId1 = RestUtil.userId();
	String userkey = RestUtil.userkey();
	Object uke;
	@BeforeClass
	public void updateUser() {
		...
		requestParams1.put("userkey", userkey);
	...
	@Test
	public void TC__02_Update_user_by_userkey() {
		String responseBody = response.getBody().asString();
		System.out.println("res"+responseBody);
		
		//System.out.println("RESULT="+object);
		//test.log(LogStatus.INFO, "Before update userkey==> "+uke);
		//Assert.assertEquals(responseBody.contains("USER_NOT_EXIST"), true);		
		//Assert.assertEquals(responseBody.contains(contactPhone), true);
		test.log(LogStatus.INFO, "Response Body is==> "+responseBody);
		test.log(LogStatus.INFO, "After update userkey==> "+userkey);
